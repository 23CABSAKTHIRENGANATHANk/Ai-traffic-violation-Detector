...
const Admin = () => {
    ...
    const fetchViolations = useCallback(async (silent = false) => {
        ...
        try {
            ...
            const url = new URL(API_CONFIG.ENDPOINTS.VIOLATIONS, window.location.origin);
            ...
            // Add mock data if result is empty
            if (merged.length === 0) {
            ...
            setIsDemo(merged.length === 0 || (apiData.length === 0 && merged.some(m => MOCK_VIOLATIONS.find(mock => mock.id === m.id))));
        ...
        try {
            ...
            if (!isDemo) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (contentType && contentType.includes('application/pdf')) {
                            ...
                            const updatedViolations = violations.map(v => 
                                v.id === violation.id ? { ...v, status: 'APPROVED' } : v
                            );
                            ...
                            // API returned data, use client-side generation
                            const response = await res.json();
                            if (response.success) {
                    ...
                    console.warn("Backend PDF generation not available:", e.message);
                }
            }
            ...
            try {
                ...
                if (!isDemo) {
                    try {
                        ...
                        console.warn('Could not update status on backend:', e);
                    }
                }
                ...
                const localViolations = JSON.parse(localStorage.getItem('traffic_violations') || '[]');
                const updatedLocal = localViolations.map(v => 
                    v.id === violation.id ? { ...v, status: 'APPROVED' } : v
                );
            ...
            console.error("Error in generateChallan", err);
            showNotification("Failed to process challan.", 'error');
        } finally {
            setActionLoading(null);
        }
    };
